...
router.post("/:id/folgas", requireAuth(), (req, res) => {
  ...
  const folga = db.criarFolga({ medicoId: req.params.id, inicio, fim: fim||inicio, motivo, notas });
  ...
  db.criarNotificacao({
    ...
    msg: `${medico.nome} registou folga de ${inicio}${fim&&fim!==inicio?" a "+fim:""} (${motivo||"sem motivo"}). Agenda bloqueada.`,
  });
